...
function removeFromFavorite(id) {
  //avoid input empty null
  if (!movies) return alert('Your favorite list is empty!!')

  //user item's index to remove the movie from the favorite movie list
  const movieIndex = movies.findIndex(movie => movie.id === id)
  if (movieIndex === -1) return alert('Cannot find the movie in your favorite list !(Index is -1.)')
